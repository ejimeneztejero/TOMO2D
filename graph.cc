...
void GraphSolver2d::solve(const Point2d& s)
{
    ...
    int Nsrc;
    // if in water
    if (smesh.inWater(src)){
        int cur = smesh.nearest(src);
        findRayExitPoint(src,cur);
        Nsrc = cur;
    }else{
        Nsrc = smesh.nearest(src);
    }
    // initialization
    B.resize(0);
    prev_node(Nsrc) = Nsrc;
...
void GraphSolver2d::pickPathRcvW(const Point2d& rcv, Array1d<Point2d>& path,
				      int& i0, int& i1) const
{
//	cerr << "pickPathRcvW" << "\n";
    if (!is_solved) error("GraphSolver2d::not yet solved");
...
void GraphSolver2d::pickReflPathRcvW(const Point2d& rcv, Array1d<Point2d>& path,
					  int& i0, int& i1, int& ir0, int& ir1) const
{
//	cerr << "pickReflPathRcvW" << "\n";
    if (!is_refl_solved) error("GraphSolver2d::not yet solved");
...
////////////////////////////////////// Slaven
void GraphSolver2d::pickPathSrcW(const Point2d& rcv, Array1d<Point2d>& path,
				      int& is0, int& is1) const
{
    ...
    //int cursrc = smesh.nearest(src);
    //findRayExitPoint(src,cursrc,ttime);
    //Point2d exitp=smesh.nodePos(cursrc);
    Point2d exitp=smesh.nodePos(cur);
    //
    if (nadd>0){
	tmp_path.pop_back();
    }
    if (nadd==1){
	// insert mid point for later bending refinement
	tmp_path.push_back(0.5*(rcv+exitp));
    }
...
void GraphSolver2d::pickPathSrcWRcvW(const Point2d& rcv, Array1d<Point2d>& path,
				      int& i0, int& i1, int& is0, int& is1) const
{
//	cerr << "pickPathSrcWrcvW" << "\n";
    if (!is_solved) error("GraphSolver2d::not yet solved");
    ...
    if (nadd==1){
	// insert mid point for later bending refinement
	tmp_path.push_back(0.5*(entryp+exitp));
    }
...
void GraphSolver2d::pickReflPathSrcW(const Point2d& rcv, Array1d<Point2d>& path,
					  int& is0, int& is1, int& ir0, int& ir1) const
{
	//cerr << "pickReflPathSrcW" << "\n";
    if (!is_refl_solved) error("GraphSolver2d::not yet solved");
    ...
    //int cursrc = smesh.nearest(src);
    //findRayExitPoint(src,cursrc,ttime_down);
    Point2d exitp=smesh.nodePos(cur);
...
void GraphSolver2d::pickReflPathSrcWRcvW(const Point2d& rcv, Array1d<Point2d>& path,
					  int& i0, int& i1, int& is0, int& is1, int& ir0, int& ir1) const
{
//	cerr << "pickReflPathSrcWRcvW" << "\n";
    if (!is_refl_solved) error("GraphSolver2d::not yet solved");
...
///////////////////////// Slaven
void GraphSolver2d::findRayExitPoint(const Point2d& src, int& cur) const
{
    Point2d p0 = smesh.nodePos(cur);
    double pwater = smesh.atWater();
    ...
    // local grid search for best ray entry point (+/- 10km radius)
    Point2d pmin(src.x()-local_search_radius,src.y());
    Point2d pmax(src.x()+local_search_radius,src.y()); 
    Index2d indmin = smesh.nodeIndex(smesh.nearest(pmin)); 
    Index2d indmax = smesh.nodeIndex(smesh.nearest(pmax)); 
    for (int i=indmin.i()+1; i<indmax.i(); i++){
	int inode = smesh.nodeIndex(i,1);
	Point2d ip = smesh.nodePos(inode);
