...
void TomographicInversion2d::solve(int niter)
{
	...
	*log_os_p << "# LSQR " << LSQR_ATOL << endl;
	...
	reset_kernel();//modified, there was a bug
	nodev_hit=0;
	...
	for (int isrc=1; isrc<=src.size(); isrc++){
	    ...
	    // ?? maybe here
	    //if (smesh.inWater(src(isrc))){
	    //    int cur = smesh.nearest(src(isrc));
	    //    smesh.findRayExitPoint(src(isrc),cur);
	    //    src2d = smesh.nodePos(cur);
	    //    if (verbose_level>=0){
	    //        cerr << "Source in the water layer.\n";
...
///////////////////////////////////////////////////////////////////////////////
		    // if receiver is in water and source si NOT:
		    if (smesh.inWater(r) && !smesh.inWater(src(isrc))){
		        //If receiver in water, mark with '*'
		        if (verbose_level>=0) cerr << "*";
		        int i0, i1;
			    graph.pickPathRcvW(r,path,i0,i1);
			...
			// if receiver is NOT in water and source is:
		    }else if (!smesh.inWater(r) && smesh.inWater(src(isrc))){
		        //If source in water, mark with '^'
		        if (verbose_level>=0) cerr << "^";
		        int is0, is1;
		        graph.pickPathSrcW(r,path,is0,is1);
		        start_i.resize(1); end_i.resize(1); interp.resize(1);
			    start_i(1) = is0; end_i(1) = is1; interp(1) = bathyp;
			    iterbend=bend.refine(path,orig_t,final_t,bend_nfac);
			    if (verbose_level>=1) cerr << "(" << iterbend << ")";
			// BOTH in WATER:
			}else if (smesh.inWater(r) && smesh.inWater(src(isrc))){
			    //If both in water, mark with '~'
		        if (verbose_level>=0) cerr << "~";
		        int i0, i1, is0, is1;
			    graph.pickPathSrcWRcvW(r,path,i0,i1,is0,is1);
			    start_i.resize(2); end_i.resize(2); interp.resize(2);
			    start_i(1) = i0; end_i(1) = i1; interp(1) = bathyp;
			    start_i(2) = is0; end_i(2) = is1; interp(2) = bathyp;
			    iterbend=bend.refine(path,orig_t,final_t,
					     start_i, end_i, interp);
			    if (verbose_level>=1) cerr << "(" << iterbend << ")";
			// NONE in WATER:
...
//////////////////////////////////////////////////////////////////////////////
            int i0,i1,is0,is1,ir0,ir1;
		    // if receiver is in water and source si NOT:
		    if (smesh.inWater(r) && !smesh.inWater(src(isrc))){
		        //If receiver in water, mark with "#"
		        if (verbose_level>=0) cerr << "#";
//			    int i0, i1, ir0, ir1;
			    graph.pickReflPathRcvW(r,path,i0,i1,ir0,ir1);
				start_i.resize(2); end_i.resize(2); interp.resize(2);
				start_i(1) = i0; end_i(1) = i1; interp(1) = bathyp;
		    ...
		    // if receiver is NOT in water and source is:
		        ...
		        if (verbose_level>=0) cerr << "+";
//		        int is0, is1, ir0, ir1;
		        graph.pickReflPathSrcW(r,path,is0,is1,ir0,ir1);
				start_i.resize(2); end_i.resize(2); interp.resize(2);
			...
			// BOTH in WATER:
			     ...
			     if (verbose_level>=0) cerr << "|";
//			     int i0, i1, is0, is1, ir0, ir1;
				 graph.pickReflPathSrcWRcvW(r,path,i0,i1,is0,is1,ir0,ir1);
				 start_i.resize(3); end_i.resize(3); interp.resize(3);
		         start_i(1) = i0; end_i(1) = i1; interp(1) = bathyp;
			     start_i(2) = ir0; end_i(2) = ir1; interp(2) = reflp;
				 start_i(3) = is0; end_i(3) = is1; interp(3) = bathyp;
				 int iterbend=bend.refine(path,orig_t,final_t,
				   			  start_i, end_i, interp);
				 if (verbose_level>=1) cerr << "(" << iterbend << ")";
			// NONE in WATER
...
// print DWS each iteration
	...
	char transfn3[MaxStr];
	sprintf(transfn3, "%s.dws.%d.%d", out_root, iter, iset);
	ofstream os3(transfn3);	
	smesh.printMaskGrid(os3, dws);
...
void TomographicInversion2d::reset_kernel()
{
    ...
    for (int i=1; i<=A.size(); i++) A(i).erase(A(i).begin(),A(i).end());
    for (int i=1; i<=Rv_h.size(); i++) Rv_h(i).erase(Rv_h(i).begin(),Rv_h(i).end());
    for (int i=1; i<=Rv_v.size(); i++) Rv_v(i).erase(Rv_v(i).begin(),Rv_v(i).end());
    for (int i=1; i<=Tv.size(); i++) Tv(i).erase(Tv(i).begin(),Tv(i).end());

    if (nrefl>0){
	for (int i=1; i<=Rd.size(); i++) Rd(i).erase(Rd(i).begin(),Rd(i).end());
	for (int i=1; i<=Td.size(); i++) Td(i).erase(Td(i).begin(),Td(i).end());
    }

    if (gravity){
	for (int i=1; i<=B.size(); i++) B(i).erase(B(i).begin(),B(i).end());
    }
