...
integer			:: i,j,k,icount,nr,art,err,k1,k2
...
character(len=200) 	:: arg,file_path
character(len=200)	:: vel_file,refl_file,dws_file
character(len=200)	:: dx_str,dz_str,dt_str,rx_str,rz_str,rt_str,rv_str,rd_str
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  do i = 1, icount	!command_argument_count()
    ...
    ! Check for switches and assign values accordingly
    select case (arg)
    case ('-v')
      call getarg(i + 1, file_path)
	if (INDEX(file_path, '"') == 1 .and. INDEX(file_path, '"', BACK=.TRUE.) == LEN_TRIM(file_path)) then
	    file_path = ADJUSTL(file_path(2:LEN_TRIM(file_path)-1))
	endif
	! Remove any remaining leading whitespace
	vel_file = ADJUSTL(file_path)
    case ('-r')
      call getarg(i + 1, file_path)
	if (INDEX(file_path, '"') == 1 .and. INDEX(file_path, '"', BACK=.TRUE.) == LEN_TRIM(file_path)) then
	    file_path = ADJUSTL(file_path(2:LEN_TRIM(file_path)-1))
	...
	! Remove any remaining leading whitespace
	dws_file = ADJUSTL(file_path)
    case ('-dx')
      call getarg(i + 1, dx_str)
      read(dx_str, *) resox
    case ('-dz')
      call getarg(i + 1, dz_str)
      read(dz_str, *) resoz
    case ('-dt')
      call getarg(i + 1, dt_str)
      read(dt_str, *) resot

    case ('-rx')
      call getarg(i + 1, rx_str)
      read(rx_str, *) numDec_x

    case ('-rz')
      call getarg(i + 1, rz_str)
      read(rz_str, *) numDec_z

    case ('-rt')
      call getarg(i + 1, rt_str)
      read(rt_str, *) numDec_t

    case ('-rv')
      call getarg(i + 1, rv_str)
      read(rv_str, *) numDec_v

    case ('-rd')
      call getarg(i + 1, rd_str)
      read(rd_str, *) numDec_d
  ...
  INQUIRE(file=trim(adjustl(refl_file)),EXIST=refl_exist)
...
!! READ REFLECTOR FILE
	  ...
	  open(10, file=trim(adjustl(refl_file)), status='old')
	  nr=0
	  do
	  	read(10,*, END=20)
		nr = nr+1
	...
	20 close(10)
	  ...
	  open(10, file=refl_file, status='old')
	  do i=1,nr
	  	read(10,*) xr(i),refl(i)
	  enddo
	  close(10)
...
if(dws_exist)	then
  ...
  if(refl_exist)write(*,*)' Name of refl model: ',trim(adjustl(refl_file))
  ...
  if(refl_exist)write(*,*)' Reflector minimum resolution in Z is: ',minr,' km'
  write(*,*)' **********************************************'
  write(*,*)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  if(refl_exist)write(*,*)"Your refl file in depth will occupy about ",sizefile," GB"
  ...
  if(refl_exist)	then
	  ...
	  file_out="reflDepth_dx" //trim(adjustl(dx_str))// "km_dz" //trim(adjustl(dz_str))//"km.out"
	  open(12, file=file_out, status='unknown')
	  do i=1,nxtotal
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  sizefile=nxtotal*(8+1)*2/1024./1024./1024.
  if(refl_exist)write(*,*)"Your refl file in TWT will occupy about ",sizefile," GB"
...
write(*,*)
  ...
  if(refl_exist) deallocate(xr,refl,reflxz,refl_time)
